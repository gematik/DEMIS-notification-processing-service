...
class XmlSanitizerTest {
  ...
  static Stream<Integer> invalidUnicodeCodepoints() {
    return Stream.of(
  ...
  @ParameterizedTest
  @MethodSource("invalidUnicodeCodepoints")
  void filterInvalidXmlCodePoints(final Integer invalidUnicodeCodepoint) {
    final String invalidUnicode = new String(Character.toChars(invalidUnicodeCodepoint));
    String xml =
    ...
    final String filteredXml = XmlSanitizer.filterInvalidXmlCodePoints(xml);
    String expected =
        "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
            + "<message>"
    ...
    assertThat(filteredXml).isEqualTo(expected);
  }
}
